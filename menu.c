...
#define LONGITUD_TRAMA 64

unsigned char tramas[MAX_TRAMAS][LONGITUD_TRAMA];
void impresiontrama( unsigned char trama[], unsigned int longitud );
int main(){
...
do{
    printf("Que trama quieres imprimir entre 1 y %d \n", MAX_TRAMAS);
     scanf("%u",&opcion);

    if ( 1 > opcion  || opcion > MAX_TRAMAS)
{
    printf("Opcion no valida ingrese un valor entre  1 y %d ", MAX_TRAMAS);
}


}while(1 > opcion  || opcion > MAX_TRAMAS);
impresiontrama(tramas[opcion - 1], LONGITUD_TRAMA);
return 0;
}
...
void impresiontrama(unsigned char trama[], unsigned int longitud) {
    printf("La trama seleccionada es: \n");
    for (unsigned int i = 0; i<LONGITUD_TRAMA; i++){
        //impresion del formato de la trama
        printf("%02x ",trama[i]);
        //Formato 
       if ((i + 1) % 16 == 0) {
            printf("\n");
